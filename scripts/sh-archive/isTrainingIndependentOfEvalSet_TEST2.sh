...
CONF=isTrainingIndependentOfEvalSet_TEST2.yaml
MODEL=data/inner/onmt/run/model_is_training_independent_of_eval_set_TEST2_step_25000.pt
DEVPRED="data/log/evaluate_models/onmt/is_training_indep_of_eval_set_TEST2_on_DEV_$CURDATE.txt"
TESTPRED="data/log/evaluate_models/onmt/is_training_indep_of_eval_set_TEST2_on_TEST_$CURDATE.txt"
...
.venv/bin/python3 $EVALSCRIPT --name isTrainingIndepOfEvalSet_TEST2_on_DEV  --pred $DEVPRED --gold $DEVGOLD

.venv/bin/python3 $EVALSCRIPT --name isTrainingIndepOfEvalSet_TEST2_on_TEST --pred $TESTPRED --gold $TESTGOLD
