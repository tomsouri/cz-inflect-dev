...
#DATAPATHS="{''corpus_1'': {''path_src'': ''data/cleaned/"$DATAROOT"/train.src'', ''path_tgt'':''data/cleaned/"$DATAROOT"/train.tgt'', ''transforms'': [''filtertoolong'']}, ''valid'': {''path_src'': ''data/cleaned/"$DATAROOT"/dev_medium.src'', ''path_tgt'': ''data/cleaned/"$DATAROOT"/dev_medium.tgt'',   ''transforms'': [''filtertoolong'']}}"

mkdir -p $LOGDIR

cat $CONFIG | sed "s=data/cleaned/neural=data/cleaned/$DATAROOT=g" > $LOGDIR"config.yaml"
