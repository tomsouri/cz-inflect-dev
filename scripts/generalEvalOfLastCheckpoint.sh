...
DEVPRED="$LOGDIR""$DEV_PREFIX""_predictions.txt"
...
.venv/bin/python3 $EVALSCRIPT --name "$NAME""_$DEV_PREFIX"  --pred $DEVPRED --gold $DEVGOLD --logdir $LOGDIR --train_steps $STEPS
...
# #Copy the checkpoints from previous training
