...
class Model(tf.keras.Model):
    ...
    class MaskElements(tf.keras.layers.Layer):
        ...
        def call(self, inputs: tf.RaggedTensor, training: bool) -> tf.RaggedTensor:
            if training:
                rand_numbers = tf.random.uniform(shape=(len(inputs.values),))
                bool_mask = rand_numbers >= self._rate
                mask = tf.cast(bool_mask, dtype=tf.int64)
                ragged_mask = inputs.with_values(mask)
                masked_inputs = inputs*ragged_mask
    ...
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        masked_words = Model.MaskElements(rate=args.word_masking)(mapped_words)
        ...
        embedded_words_we = tf.keras.layers.Embedding(input_dim=train.forms.word_mapping.vocabulary_size(), output_dim=args.we_dim)(masked_words)
        ...
        flattened = words.values
        unique_words, indices_of_unique_words = tf.unique(flattened)
        ...
        letter_sequences = tf.strings.unicode_split(unique_words, input_encoding="UTF-8")

        # TODO: Map the letters into ids by using `char_mapping` of `train.forms`.
        mapped_letters = train.forms.char_mapping(letter_sequences)

        # TODO: Embed the input characters with dimensionality `args.cle_dim`.
        embedded_letters = tf.keras.layers.Embedding(input_dim=train.forms.char_mapping.vocabulary_size(), output_dim=args.cle_dim)(mapped_letters)
        ...
        gru_layer = tf.keras.layers.GRU(units=args.cle_dim)
        gru_bidirectional = tf.keras.layers.Bidirectional(gru_layer,merge_mode="concat")(embedded_letters)
        ...
        cl_repre_of_flattend_words = tf.gather(gru_bidirectional, indices=indices_of_unique_words)
        ...
        embedded_words_cle = words.with_values(cl_repre_of_flattend_words)

        # TODO: Concatenate the word-level embeddings and the computed character-level WEs
        # (in this order).

        embedded = tf.concat([embedded_words_we,embedded_words_cle], axis=-1)
        ...
        rnn = tf.keras.layers.Bidirectional(rnn_cell, merge_mode='sum')(embedded)
        ...
        predictions = tf.keras.layers.Dense(units=train.tags.word_mapping.vocabulary_size(), activation=tf.nn.softmax)(rnn)
...
def main(args: argparse.Namespace) -> Dict[str, float]:
    ...
    def extract_tagging_data(example):
        forms = example["forms"]
        tags  = morpho.train.tags.word_mapping(example["tags"])
        return (forms,tags)
